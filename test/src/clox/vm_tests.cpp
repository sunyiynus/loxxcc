...
TEST(ChunkTest, AppendOpFromRawArray) {
    chunk testChunk;

    // 用raw数组添加操作码
    uint8_t rawOp[BYTECODE_LEN] = {static_cast<uint8_t>(op_type::OP_ADD), 10};
    testChunk.appendOp(rawOp);

    // 检查是否插入成功
    EXPECT_EQ(testChunk.ops.size(), 1);
    EXPECT_EQ(testChunk.readOp(0), static_cast<uint8_t>(op_type::OP_ADD));
    EXPECT_EQ(testChunk.readOprand(0), 10);
}
